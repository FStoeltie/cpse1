...
TEST_CASE( "Testing char array 4_4" ){
    Set<std::array<char, 3>, 3> s;
    std::array<char, 3> test = {'a', 'b', 'c'};
    // std::cout << "test array: " << test << std::endl;
    REQUIRE_THROWS( s.max());
    s.add({'c', 'b', 'c'});
    s.add({'a', 'b', 'c'});
    s.add({'c', 'b', 'd'});
    REQUIRE( s.max() == std::array<char, 3>{'c', 'b', 'd'});
    std::cout << "char array set: " << s << std::endl;
}